...
<template>
  <div class="flex h-40 items-end gap-1">
    <div v-for="(_, index) in data" :key="index" ref="barsRef" class="bg-jarvis-primary w-4" />
  </div>
</template>
```
...
timeline.to(element, { opacity: 1, duration: 0.3 }).to(element, { y: -20, duration: 0.5 });
...
const showBad = () => {
  gsap.timeline().to(element, { opacity: 1, duration: 0.3 }).to(element, { y: -20, duration: 0.5 });
};
```
