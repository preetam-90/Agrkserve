...
import { Calendar, Clock, ArrowRight, Sparkles, BookOpen, ChevronRight, Star } from 'lucide-react';
...
export default async function BlogPage({
  ...
  searchParams: Promise<{ page?: string; category?: string } | undefined>;
}) {
  ...
  return (
    <div className="min-h-screen bg-[#030705] text-white selection:bg-emerald-500/30">
      ...
      <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
        ...
        <div className="absolute bottom-0 right-0 h-[600px] w-[600px] rounded-full bg-cyan-500/5 blur-[120px]" />
      </div>

      {/* Hero Section */}
      <section className="relative z-10 px-4 pb-12 pt-32">
        <div className="mx-auto max-w-7xl text-center">
          <Badge
            variant="outline"
            className="mb-6 rounded-full border-emerald-500/30 bg-emerald-500/10 px-4 py-1 text-emerald-400 transition-colors hover:bg-emerald-500/20"
          >
          ...
          <h1 className="mb-6 text-5xl font-extrabold tracking-tight md:text-7xl">
            ...
            , <br className="hidden md:block" /> Not Just Harder.
          </h1>
          <p className="mx-auto max-w-2xl text-lg leading-relaxed text-white/60 md:text-xl">
            Expert advice on farm mechanization, crop management, and the business of agriculture.
            Written for the modern Indian farmer.
          </p>

          {/* Newsletter CTA Component */}
          <div className="group relative mx-auto mt-10 max-w-md">
            <div className="absolute -inset-1 rounded-lg bg-gradient-to-r from-emerald-500 to-cyan-500 opacity-25 blur transition duration-1000 group-hover:opacity-50 group-hover:duration-200"></div>
            <div className="relative flex items-center rounded-lg border border-white/10 bg-[#0A1A16] p-1.5">
              <Input
                type="email"
                placeholder="Enter your email for weekly tips..."
                className="h-10 border-none bg-transparent text-white placeholder:text-white/40 focus-visible:ring-0"
              />
              <Button
                size="sm"
                className="h-10 rounded-md bg-emerald-500 px-6 font-medium text-white hover:bg-emerald-600"
              >
                Subscribe
              </Button>
            </div>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          {featuredPost && (
            <div className="mb-16">
              <h2 className="mb-6 flex items-center gap-2 text-2xl font-bold text-white">
                <Star className="h-5 w-5 fill-yellow-400 text-yellow-400" /> Featured Article
              </h2>
              <div className="group relative grid overflow-hidden rounded-3xl border border-white/10 bg-[#0A1A16] md:grid-cols-2 lg:h-[450px]">
                ...
                <div className="flex flex-col justify-center p-8 md:p-12">
                  ...
                  <Link href={`/blog/${featuredPost.id}`}>
                    <h3 className="mb-4 text-3xl font-bold leading-tight text-white transition-colors group-hover:text-emerald-400">
                      {featuredPost.title.en}
                    </h3>
                  </Link>
                  <p className="mb-6 line-clamp-3 text-lg text-white/60">{featuredPost.excerpt}</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-white/40">
                          {featuredPost.author.role || 'Writer'}
                        </p>
                      </div>
                    </div>
                    <Link href={`/blog/${featuredPost.id}`}>
                      <Button
                        variant="ghost"
                        className="text-emerald-400 transition-transform hover:bg-emerald-500/10 hover:text-emerald-300 group-hover:translate-x-1"
                      >
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {gridPosts.map((post) => (
              <article
                key={post.id}
                className="group relative flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0A1A16] transition-all duration-300 hover:border-emerald-500/30 hover:shadow-2xl hover:shadow-emerald-900/10"
              >
                {/* Image */}
                ...
                <div className="flex flex-1 flex-col p-6">
                  ...
                  <Link href={`/blog/${post.id}`} className="mb-3 block">
                    <h3 className="line-clamp-2 text-xl font-bold text-white transition-colors group-hover:text-emerald-400">
                      {post.title.en}
                    </h3>
                  </Link>

                  <p className="mb-6 line-clamp-2 flex-1 text-sm text-white/50">{post.excerpt}</p>

                  <div className="mt-auto flex items-center justify-between border-t border-white/5 pt-4">
                    <span className="text-xs font-medium text-white/60">By {post.author.name}</span>
                    <Link
                      href={`/blog/${post.id}`}
                      className="-mr-2 rounded-full p-2 text-emerald-400 transition-colors hover:bg-white/5"
                    >
          ...
          {posts.length === 0 && (
            <div className="py-20 text-center">
              <BookOpen className="mx-auto mb-4 h-16 w-16 text-white/10" />
      ...
      {/* Footer / Contact CTA */}
      <section className="relative z-10 border-t border-white/5 bg-[#030705] px-4 py-24">
        <div className="mx-auto max-w-4xl text-center">
          <h2 className="mb-6 text-3xl font-bold text-white">Have a topic you want us to cover?</h2>
