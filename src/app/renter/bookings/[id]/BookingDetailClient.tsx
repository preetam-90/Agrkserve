...
function BookingDetailPageContent({ initialBooking }: BookingDetailClientProps) {
  const params = useParams();
  const searchParams = useSearchParams();
  const bookingId = (params.id as string) || (initialBooking?.id as string);
  const isSuccess = searchParams.get('success') === 'true';

  const [booking, setBooking] = useState<
    (Booking & { equipment?: Equipment; provider?: UserProfile }) | null
  >(initialBooking || null);
  ...
  useEffect(() => {
    if (!initialBooking || (booking && booking.id !== bookingId)) {
      loadBooking();
    }
              ...
              <CardContent className="p-6">
                ...
                <div className="flex gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <Link
                      href={`/equipment/item/${booking.equipment_id}`}
...
export default function BookingDetailClient({ initialBooking }: BookingDetailClientProps) {
  return (
    ...
    >
      <BookingDetailPageContent initialBooking={initialBooking} />
    </Suspense>
  );
}
