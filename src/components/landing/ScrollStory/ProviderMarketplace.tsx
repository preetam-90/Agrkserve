...
function ProviderMarketplace({ stats }: { stats: Stats }) {
  ...
  return (
    <section
      id="provider-network"
      ref={rootRef}
      className="bg-[#080f0b] px-5 py-24 md:px-8 md:py-36"
    >
      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.25fr_1fr]">
        <div className="relative h-[420px] overflow-hidden rounded-[2rem] border border-white/10 bg-[#0a120e]">
          ...
          >
            ...
            <path
              d="M18 60 L32 42 L46 66 L59 38 L72 58"
              stroke="#22d3ee"
              strokeWidth="0.45"
              fill="none"
            />
        ...
        <div className="provider-grid grid gap-4">
          <article className="provider-card bg-white/8 rounded-3xl border border-white/10 p-7 backdrop-blur-2xl">
            <p className="text-xs uppercase tracking-[0.2em] text-cyan-200">Providers</p>
            <p
              data-provider={Math.max(stats.totalUsers, 1200)}
              className="mt-4 text-5xl font-semibold tracking-[-0.02em] text-white"
            >
              0
            </p>
          </article>
          <article className="provider-card from-emerald-500/18 rounded-3xl border border-emerald-300/20 bg-gradient-to-br to-transparent p-7 backdrop-blur-2xl">
            <p className="text-xs uppercase tracking-[0.2em] text-emerald-100">Machines</p>
            <p
              data-provider={Math.max(stats.totalEquipment, 5000)}
              className="mt-4 text-5xl font-semibold tracking-[-0.02em] text-white"
            >
              0
            </p>
          ...
          <article className="provider-card bg-white/8 rounded-3xl border border-white/10 p-7 backdrop-blur-2xl">
            ...
            <p
              data-provider={Math.max(stats.totalBookings, 23000)}
              className="mt-4 text-5xl font-semibold tracking-[-0.02em] text-white"
            >
              0
            </p>
