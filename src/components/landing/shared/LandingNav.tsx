...
export function LandingNav() {
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between rounded-2xl border border-white/12 bg-[#08100d]/72 px-4 py-3 backdrop-blur-2xl md:px-6">
        ...
        <nav className="hidden items-center gap-6 lg:flex">
          {navLinks.map((link) => (
            <Link key={link.label} href={link.href} className="text-sm text-zinc-200/90 transition hover:text-cyan-200">
              {link.label}
            </Link>
          ))}
        </nav>
