    ...
    const ctx = gsap.context(() => {
      gsap.from(targets, {
        ...
        onComplete: () => {
          ...
          (targets as NodeListOf<HTMLElement>).forEach((t) => (t.style.willChange = 'auto'));
        },
      });
    }, container);
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(selector, {
        ...
        onComplete: () => {
          gsap.utils.toArray<HTMLElement>(selector).forEach((el) => (el.style.willChange = 'auto'));
        },
      });
...
function useScrollVelocityGlow(
  containerRef: RefObject<HTMLElement | null>,
  selector: string,
...
/* ------------------------------------------------------------------ */
function useSectionMagneticPull(
  containerRef: RefObject<HTMLElement | null>,
  selector: string,
...
function useCounterScramble(
  ...
  const { reducedMotion = false, duration = 1.2, scrambleChars = '0123456789' } = options;
            ...
            onUpdate: () => {
              ...
              digitIndices.forEach((idx) => {
                if (Math.random() > obj.progress) {
                  result[idx] = scrambleChars[Math.floor(Math.random() * scrambleChars.length)];
                }
              });
