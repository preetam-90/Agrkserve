...
function Calendar({
  ...
  const mergedModifierClassNames = {
    booked: '!bg-rose-500/10 !text-rose-400 font-semibold ring-1 ring-rose-500/30 !rounded-xl',
  ...
  return (
    <div
      className={cn(
        'mx-auto w-fit rounded-[3rem] border border-emerald-500/10 bg-[#040a16]/80 p-8 backdrop-blur-3xl',
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 opacity-20">
        <div className="absolute right-0 top-0 h-full w-full bg-[radial-gradient(circle_at_70%_10%,#10b98115_0%,transparent_50%)]" />
        <div className="absolute bottom-0 left-0 h-full w-full bg-[radial-gradient(circle_at_20%_90%,#06b6d410_0%,transparent_50%)]" />
      </div>

      {/* Dynamic background glows */}
      <div className="pointer-events-none absolute -left-32 -top-32 h-64 w-64 bg-emerald-500/10 blur-[100px] transition-colors duration-1000 group-hover:bg-emerald-500/20" />
      <div className="pointer-events-none absolute -bottom-32 -right-32 h-64 w-64 bg-emerald-500/10 blur-[100px] transition-colors duration-1000 group-hover:bg-emerald-500/20" />
      ...
      <DayPicker
        ...
        classNames={{
          ...
          caption_label:
            'text-lg font-black tracking-[0.25em] uppercase text-emerald-50 drop-shadow-[0_0_15px_rgba(255,255,255,0.1)]',
        ...
        components={{
          Chevron: ({ orientation }) => {
            ...
            return (
              <Icon className="h-6 w-6 transition-transform duration-300 group-hover/nav:scale-110" />
            );
          },
        }}
        footer={
          <div className="mt-12 flex flex-wrap items-center justify-center gap-x-8 gap-y-4 border-t border-white/5 pt-10">
            <Legend
...
function Legend({ color, label }: { color: string; label: string }) {
  return (
    <span className="flex items-center gap-2.5">
      ...
      <span className="whitespace-nowrap text-[10px] font-black uppercase tracking-[0.15em] text-white/35">
        {label}
      </span>
    </span>
  );
